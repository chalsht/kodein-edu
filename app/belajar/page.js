            ...
            {data.materi?.length ? (
              data.materi.map((item, i) => (
                <div
                  ...
                  style={{
                    animation: `fadeIn .5s ease ${i * 0.1}s both`,
                  }}
                >
...
function MenuCard({ href, icon, title, text, color }) {
  ...
  return (
    <Link
      href={href}
      className={`group bg-white ${c.border} border rounded-2xl p-6 shadow-sm hover:shadow-lg hover:-translate-y-2 transition-all duration-300`}
    >
      <div className="flex justify-between items-center">

        <div
          className={`w-12 h-12 ${c.bg} rounded-2xl flex items-center justify-center text-2xl group-hover:scale-110 transition`}
        >
          {icon}
        </div>

        <div
          className={`w-9 h-9 rounded-full ${c.bg} ${c.text} flex items-center justify-center font-bold ${c.hover} group-hover:text-white transition`}
        >
          →
        </div>
